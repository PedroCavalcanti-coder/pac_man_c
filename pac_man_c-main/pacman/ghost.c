...
void ghost()
{
    ...
    int ny = py, nx = px;
    if (py < py_pacman && py_pacman-py > px_pacman-px || py_pacman-py > px-px_pacman)
    {
    ...
    else if (py > py_pacman && py-py_pacman > px-px_pacman || py-py_pacman > px_pacman-px)
    {
        if (mapa[py - 1][px] != '#') // Se o estado para onde quero ir não for uma parede
        {
            ny = py - 1;
            ...
            else if(mapa[py_anterior][px_anterior] == ' ')
            {
                // Para evitar bugs
                mapa[py][px] = ' ';
            }
    ...
    if (px < px_pacman && px_pacman-px > py_pacman-py || px_pacman-px > py-py_pacman)
    {
    ...
    else if (px > px_pacman && px_pacman-px > py_pacman-py || px-px_pacman > py-py_pacman)
    {
        if (mapa[py][px - 1] != '#') // Se o estado para onde quero ir não for uma parede
        {
            nx = px - 1;
